...
# Utility to parse optional date strings
...
def get_member(id):
    ...
    return (jsonify(m.to_dict()) if m else (jsonify({'msg':'Not found'}),404))
...
def get_property(id):
    ...
    return (jsonify(p.to_dict()) if p else (jsonify({'msg':'Not found'}),404))
...
def update_property(id):
    ...
    p.address = d['address']; p.suburb = d['suburb']; p.purpose = d['purpose']
    db.session.commit()
...
def get_account(id):
    ...
    return (jsonify(a.to_dict()) if a else (jsonify({'msg':'Not found'}),404))
...
def get_transaction(id):
    ...
    return (jsonify(t.to_dict()) if t else (jsonify({'msg':'Not found'}),404))

@app.route('/api/transactions', methods=['POST'])
def create_transaction():
    d = request.get_json()
    td = parse_date(d.get('transactiondate'))
    t = Transaction(**{k: d.get(k) for k in ('billid','purchaseid','category','subcategory1','subcategory2','subcategory3','provider','amount','accountid','propertyid')}, transactiondate=td)
...
def create_purchase():
    ...
    p = Purchase(**{k: d.get(k) for k in ('transactionid','memberid','provider','address','category','subcategory1','subcategory2','subcategory3','accountid','amount')}, purchasedate=pd)
    db.session.add(p); db.session.commit()
    return jsonify(p.to_dict()), 201
...
def get_purchased_item(id):
    ...
    return (jsonify(pi.to_dict()) if pi else (jsonify({'msg':'Not found'}),404))
...
def delete_purchased_item(id):
    pi = PurchasedItem.query.get(id)
    if not pi: return jsonify({'msg':'Not found'}),404
    db.session.delete(pi); db.session.commit()
    return '', 204
...
def get_bill(id):
    b = Bill.query.get(id)
    return (jsonify(b.to_dict()) if b else (jsonify({'msg':'Not found'}),404))
...
def get_category(id):
    ...
    return (jsonify(c.to_dict()) if c else (jsonify({'msg':'Not found'}),404))
